...
class RobDroneControl():
    ...
    def vicon_callback(self, vicon_pose: TransformStamped):
        ...
        self.broadcaster.sendTransform(numpy_to_transform_stamped(self.t_map_global, frame_id='map', child_frame_id='global', time = rospy.Time.now()))
    ...
    def launch_cb(self, request: Empty):
        ...
        if not BUILD_MAP:
            ...
            for angle in np.linspace(yaw, yaw - 2*np.pi, 4):
                self.waypoint_queue_push(config_to_pose_stamped(x + r * np.cos(angle), y + r * np.sin(angle), self.launch_height, angle, self.current_t_map_dots.header.frame_id))
